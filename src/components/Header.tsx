...
export default function Header() {
  ...
  const selectedProjectName = projects.find(p => p.id === currentProject)?.name || '프로젝트 선택'
  ...
  return (
    <header className="h-16 border-b border-[#222631] bg-[#090A0D]/80 backdrop-blur flex items-center justify-between px-6 md:px-8 z-50 sticky top-0 select-none">
      <div className="flex items-center gap-6">
        {/* Logo */}
        <Link href="/" className="text-lg font-black tracking-wider text-white hover:opacity-90 transition">
              ...
              <div className="absolute left-0 mt-2 w-56 rounded-2xl bg-[#090A0D] border border-[#222631] shadow-2xl py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-150">
                ...
                {projects.map((proj) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-2 text-xs font-semibold flex items-center justify-between transition cursor-pointer hover:bg-zinc-800 ${
                      currentProject === proj.id ? 'text-[#00BA54] bg-[#00BA54]/5' : 'text-zinc-300'
                    }`}
                  >
                    <span>{proj.name}</span>
                    {currentProject === proj.id && <span className="text-[#00BA54] text-[10px]">●</span>}
                  </button>
                ))}
                ...
                <button
                  ...
                  className={`w-full text-left px-4 py-2 text-xs font-semibold flex items-center gap-2 transition cursor-pointer hover:bg-zinc-800 ${
                    !currentProject ? 'text-blue-400 bg-blue-500/5' : 'text-zinc-400'
                  }`}
                >
      ...
      <div className="flex items-center gap-4 text-xs font-semibold text-zinc-400">
        <span className="hidden sm:inline">협업 스페이스 (2인 개발 모드)</span>
        <div className="flex items-center gap-2 bg-zinc-950/40 border border-zinc-800/60 rounded-full py-1 px-3">
          <span className="w-1.5 h-1.5 rounded-full bg-[#00BA54] animate-pulse" />
          <span className="text-[10px] font-bold text-zinc-400 font-mono">준비됨</span>
        </div>
      </div>
    </header>
  )
